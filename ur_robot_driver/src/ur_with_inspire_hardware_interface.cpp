...
#include <ur_robot_driver/ur_with_inspire_hardware_interface.h>
...
URwInspireHardwareInterface::URwInspireHardwareInterface()
...
bool URwInspireHardwareInterface::init(ros::NodeHandle& root_nh, ros::NodeHandle& robot_hw_nh)
{
  joint_velocities_ = { { 0, 0, 0, 0, 0, 0} };
  joint_efforts_ = { { 0, 0, 0, 0, 0, 0} };
  ...
  try
  {
    ur_driver_.reset(new urcl::UrDriver(
        ...
        std::bind(&URwInspireHardwareInterface::handleRobotProgramState, this, std::placeholders::_1), headless_mode,
  ...
  ur_driver_->registerTrajectoryDoneCallback(
      std::bind(&URwInspireHardwareInterface::passthroughTrajectoryDoneCb, this, std::placeholders::_1));
  ...
  jnt_traj_interface_.registerGoalCallback(
      std::bind(&URwInspireHardwareInterface::startJointInterpolation, this, std::placeholders::_1));
  jnt_traj_interface_.registerCancelCallback(std::bind(&URwInspireHardwareInterface::cancelInterpolation, this));
  cart_traj_interface_.registerGoalCallback(
      std::bind(&URwInspireHardwareInterface::startCartesianInterpolation, this, std::placeholders::_1));
  cart_traj_interface_.registerCancelCallback(std::bind(&URwInspireHardwareInterface::cancelInterpolation, this));
  ...
  set_speed_slider_srv_ = robot_hw_nh.advertiseService("set_speed_slider", &URwInspireHardwareInterface::setSpeedSlider, this);

  // Service to set any of the robot's IOs
  set_io_srv_ = robot_hw_nh.advertiseService("set_io", &URwInspireHardwareInterface::setIO, this);
  ...
  if (headless_mode)
  {
    ...
    resend_robot_program_srv_ =
        robot_hw_nh.advertiseService("resend_robot_program", &URwInspireHardwareInterface::resendRobotProgram, this);
  }

  // Calling this service will make the "External Control" program node on the UR-Program return.
  deactivate_srv_ = robot_hw_nh.advertiseService("hand_back_control", &URwInspireHardwareInterface::stopControl, this);

  // Calling this service will zero the robot's ftsensor (only available for e-Series).
  tare_sensor_srv_ = robot_hw_nh.advertiseService("zero_ftsensor", &URwInspireHardwareInterface::zeroFTSensor, this);

  // Setup the mounted payload through a ROS service
  set_payload_srv_ = robot_hw_nh.advertiseService("set_payload", &URwInspireHardwareInterface::setPayload, this);
...
template <typename T>
void URwInspireHardwareInterface::readData(const std::unique_ptr<rtde::DataPackage>& data_pkg, const std::string& var_name,
                                 T& data)
{
...
void URwInspireHardwareInterface::readBitsetData(const std::unique_ptr<rtde::DataPackage>& data_pkg, const std::string& var_name,
                                       std::bitset<N>& data)
{
...
void URwInspireHardwareInterface::read(const ros::Time& time, const ros::Duration& period)
{
  // set defaults
...
bool URwInspireHardwareInterface::prepareSwitch(const std::list<hardware_interface::ControllerInfo>& start_list,
...
void URwInspireHardwareInterface::doSwitch(const std::list<hardware_interface::ControllerInfo>& start_list,
                                 const std::list<hardware_interface::ControllerInfo>& stop_list)
{
...
uint32_t URwInspireHardwareInterface::getControlFrequency() const
{
...
void URwInspireHardwareInterface::transformForceTorque()
{
...
bool URwInspireHardwareInterface::isRobotProgramRunning() const
{
  return robot_program_running_;
}

void URwInspireHardwareInterface::handleRobotProgramState(bool program_running)
{
...
bool URwInspireHardwareInterface::shouldResetControllers()
{
...
void URwInspireHardwareInterface::extractToolPose(const ros::Time& timestamp)
{
...
void URwInspireHardwareInterface::publishPose()
{
  if (tcp_pose_pub_)
  {
...
void URwInspireHardwareInterface::extractRobotStatus()
{
...
void URwInspireHardwareInterface::publishIOData()
{
  if (io_pub_)
  {
    if (io_pub_->trylock())
...
void URwInspireHardwareInterface::publishToolData()
{
  if (tool_data_pub_)
  {
...
bool URwInspireHardwareInterface::stopControl(std_srvs::TriggerRequest& req, std_srvs::TriggerResponse& res)
{
...
bool URwInspireHardwareInterface::setSpeedSlider(ur_msgs::SetSpeedSliderFractionRequest& req,
...
bool URwInspireHardwareInterface::setIO(ur_msgs::SetIORequest& req, ur_msgs::SetIOResponse& res)
{
...
bool URwInspireHardwareInterface::resendRobotProgram(std_srvs::TriggerRequest& req, std_srvs::TriggerResponse& res)
{
...
bool URwInspireHardwareInterface::zeroFTSensor(std_srvs::TriggerRequest& req, std_srvs::TriggerResponse& res)
{
...
bool URwInspireHardwareInterface::setPayload(ur_msgs::SetPayloadRequest& req, ur_msgs::SetPayloadResponse& res)
{
  urcl::vector3d_t cog;
...
void URwInspireHardwareInterface::commandCallback(const std_msgs::StringConstPtr& msg)
{
...
void URwInspireHardwareInterface::publishRobotAndSafetyMode()
{
  if (robot_mode_pub_)
  {
...
bool URwInspireHardwareInterface::checkControllerClaims(const std::set<std::string>& claimed_resources)
{
...
void URwInspireHardwareInterface::startJointInterpolation(const hardware_interface::JointTrajectory& trajectory)
{
...
void URwInspireHardwareInterface::startCartesianInterpolation(const hardware_interface::CartesianTrajectory& trajectory)
{
  size_t point_number = trajectory.trajectory.points.size();
...
void URwInspireHardwareInterface::cancelInterpolation()
{
...
void URwInspireHardwareInterface::passthroughTrajectoryDoneCb(urcl::control::TrajectoryResult result)
{
    ...
    if(power_open) {
      ...
      for(int i = 0; i < 6; i++) {
        if(fabs(inspire_hand->setangle_[i] - inspire_hand->curangle_[i]) < 0.05) {
          auto force_neg_threshold = inspire_hand->force_neg_threshold_lookup[i];
          if(inspire_hand->curangle_[i] > 0.5) {
            force_neg_threshold *= 2.0;
          }
          double force_ratio_lookup = 0.001 * (inspire_hand->angle_upper_limit[i] - inspire_hand->angle_lower_limit[i]);
          if(inspire_hand->curforce_[i] > inspire_hand->force_pos_threshold_lookup[i]) {
            inspire_hand->setangle_[i] = std::max(inspire_hand->setangle_[i] - force_ratio_lookup * (inspire_hand->curforce_[i] - inspire_hand->force_pos_threshold_lookup[i]) / 2.0 , inspire_hand->angle_lower_limit[i]);
          } else if(inspire_hand->curforce_[i] < force_neg_threshold) {
            inspire_hand->setangle_[i] = std::min(inspire_hand->setangle_[i] - force_ratio_lookup * (inspire_hand->curforce_[i] - inspire_hand->force_neg_threshold_lookup[i]), inspire_hand->angle_upper_limit[i]);
          }
        }
      }
    } else {
...
PLUGINLIB_EXPORT_CLASS(ur_driver::URwInspireHardwareInterface, hardware_interface::RobotHW)
