...
#include <ur_robot_driver/ur_with_linker_hardware_interface.h>
...
URwLinkerHardwareInterface::URwLinkerHardwareInterface()
...
bool URwLinkerHardwareInterface::init(ros::NodeHandle& root_nh, ros::NodeHandle& robot_hw_nh)
{
  joint_velocities_ = { { 0, 0, 0, 0, 0, 0} };
  joint_efforts_ = { { 0, 0, 0, 0, 0, 0} };
  ...
  try
  {
    ur_driver_.reset(new urcl::UrDriver(
        ...
        std::bind(&URwLinkerHardwareInterface::handleRobotProgramState, this, std::placeholders::_1), headless_mode,
  ...
  ur_driver_->registerTrajectoryDoneCallback(
      std::bind(&URwLinkerHardwareInterface::passthroughTrajectoryDoneCb, this, std::placeholders::_1));
  ...
  // end
  command_sub_ = robot_hw_nh.subscribe("script_command", 1, &URwLinkerHardwareInterface::commandCallback, this);

  // initialize the linker hand
  auto hand_prefix = robot_hw_nh.param<bool>("use_right_hand", true) ? "right" : "left";
  int joint_nums = robot_hw_nh.param<int>("hand_joint_nums", 10);
  linker_hand_.set_nh(&root_nh, hand_prefix, joint_nums);

  hand_control_thread_ = std::thread(handCommunicationThread, std::ref(linker_hand_), std::ref(hand_in_freedrive_), std::ref(power_open_), std::ref(power_close_));
  ...
  for (std::size_t i = 0; i < joint_names_.size(); ++i)
  {
    if(i < 6) {
      ...
      js_interface_.registerHandle(hardware_interface::JointStateHandle(joint_names_[i], &linker_hand_.curangle_[i-6], &linker_hand_.curspeed_[i-6], &linker_hand_.curforce_[i-6]));
      // Create joint position control interface
      pj_interface_.registerHandle(
          hardware_interface::JointHandle(js_interface_.getHandle(joint_names_[i]), &linker_hand_.setangle_cmd_[i-6]));
      vj_interface_.registerHandle(
          hardware_interface::JointHandle(js_interface_.getHandle(joint_names_[i]), &linker_hand_.setangle_cmd_[i-6]));
      spj_interface_.registerHandle(scaled_controllers::ScaledJointHandle(
          js_interface_.getHandle(joint_names_[i]), &linker_hand_.setangle_cmd_[i-6], &speed_scaling_combined_));
      svj_interface_.registerHandle(scaled_controllers::ScaledJointHandle(
          js_interface_.getHandle(joint_names_[i]), &linker_hand_.setangle_cmd_[i-6], &speed_scaling_combined_));    }
  }
  ...
  jnt_traj_interface_.registerGoalCallback(
      std::bind(&URwLinkerHardwareInterface::startJointInterpolation, this, std::placeholders::_1));
  jnt_traj_interface_.registerCancelCallback(std::bind(&URwLinkerHardwareInterface::cancelInterpolation, this));
  cart_traj_interface_.registerGoalCallback(
      std::bind(&URwLinkerHardwareInterface::startCartesianInterpolation, this, std::placeholders::_1));
  cart_traj_interface_.registerCancelCallback(std::bind(&URwLinkerHardwareInterface::cancelInterpolation, this));
  ...
  set_speed_slider_srv_ = robot_hw_nh.advertiseService("set_speed_slider", &URwLinkerHardwareInterface::setSpeedSlider, this);

  // Service to set any of the robot's IOs
  set_io_srv_ = robot_hw_nh.advertiseService("set_io", &URwLinkerHardwareInterface::setIO, this);
  ...
  if (headless_mode)
  {
    ...
    resend_robot_program_srv_ =
        robot_hw_nh.advertiseService("resend_robot_program", &URwLinkerHardwareInterface::resendRobotProgram, this);
  }

  // Calling this service will make the "External Control" program node on the UR-Program return.
  deactivate_srv_ = robot_hw_nh.advertiseService("hand_back_control", &URwLinkerHardwareInterface::stopControl, this);

  // Calling this service will zero the robot's ftsensor (only available for e-Series).
  tare_sensor_srv_ = robot_hw_nh.advertiseService("zero_ftsensor", &URwLinkerHardwareInterface::zeroFTSensor, this);

  // Setup the mounted payload through a ROS service
  set_payload_srv_ = robot_hw_nh.advertiseService("set_payload", &URwLinkerHardwareInterface::setPayload, this);
...
template <typename T>
void URwLinkerHardwareInterface::readData(const std::unique_ptr<rtde::DataPackage>& data_pkg, const std::string& var_name,
                                 T& data)
{
...
void URwLinkerHardwareInterface::readBitsetData(const std::unique_ptr<rtde::DataPackage>& data_pkg, const std::string& var_name,
                                       std::bitset<N>& data)
{
...
void URwLinkerHardwareInterface::read(const ros::Time& time, const ros::Duration& period)
{
  // set defaults
  ...
  if (data_pkg)
  {
    ...
    if (joint_forward_controller_running_)
    {
      ...
      for (size_t i = 0; i < linker_hand_.joint_nums_; i++) {
        feedback.desired.positions.push_back(linker_hand_.setangle_cmd_[i]);
        feedback.actual.positions.push_back(linker_hand_.curangle_[i]);
        feedback.error.positions.push_back(std::abs(linker_hand_.curangle_[i] - linker_hand_.setangle_cmd_[i]));
      }
  ...
  else
  {
    ...
    if (!non_blocking_read_)
    {
      ...
      if(error_counter++ > 50)
      {
        ...
        std::system("rosnode kill /robot_state_publisher & rosnode kill /ros_control_controller_spawner & rosnode kill /ros_control_stopped_spawner & rosnode kill /controller_stopper & rosnode kill /ur_with_linker_hardware_interface/ur_robot_state_helper & rosnode kill /ur_with_linker_hardware_interface");
      }
    }
  }
}

void URwLinkerHardwareInterface::write(const ros::Time& time, const ros::Duration& period)
{
...
bool URwLinkerHardwareInterface::prepareSwitch(const std::list<hardware_interface::ControllerInfo>& start_list,
...
void URwLinkerHardwareInterface::doSwitch(const std::list<hardware_interface::ControllerInfo>& start_list,
                                 const std::list<hardware_interface::ControllerInfo>& stop_list)
{
...
uint32_t URwLinkerHardwareInterface::getControlFrequency() const
{
...
void URwLinkerHardwareInterface::transformForceTorque()
{
...
bool URwLinkerHardwareInterface::isRobotProgramRunning() const
{
  return robot_program_running_;
}

void URwLinkerHardwareInterface::handleRobotProgramState(bool program_running)
{
...
bool URwLinkerHardwareInterface::shouldResetControllers()
{
...
void URwLinkerHardwareInterface::extractToolPose(const ros::Time& timestamp)
{
...
void URwLinkerHardwareInterface::publishPose()
{
  if (tcp_pose_pub_)
  {
...
void URwLinkerHardwareInterface::extractRobotStatus()
{
...
void URwLinkerHardwareInterface::publishIOData()
{
  if (io_pub_)
  {
    if (io_pub_->trylock())
...
void URwLinkerHardwareInterface::publishToolData()
{
  if (tool_data_pub_)
  {
...
bool URwLinkerHardwareInterface::stopControl(std_srvs::TriggerRequest& req, std_srvs::TriggerResponse& res)
{
...
bool URwLinkerHardwareInterface::setSpeedSlider(ur_msgs::SetSpeedSliderFractionRequest& req,
...
bool URwLinkerHardwareInterface::setIO(ur_msgs::SetIORequest& req, ur_msgs::SetIOResponse& res)
{
...
bool URwLinkerHardwareInterface::resendRobotProgram(std_srvs::TriggerRequest& req, std_srvs::TriggerResponse& res)
{
...
bool URwLinkerHardwareInterface::zeroFTSensor(std_srvs::TriggerRequest& req, std_srvs::TriggerResponse& res)
{
...
bool URwLinkerHardwareInterface::setPayload(ur_msgs::SetPayloadRequest& req, ur_msgs::SetPayloadResponse& res)
{
  urcl::vector3d_t cog;
...
void URwLinkerHardwareInterface::commandCallback(const std_msgs::StringConstPtr& msg)
{
...
void URwLinkerHardwareInterface::publishRobotAndSafetyMode()
{
  if (robot_mode_pub_)
  {
...
bool URwLinkerHardwareInterface::checkControllerClaims(const std::set<std::string>& claimed_resources)
{
...
void URwLinkerHardwareInterface::startJointInterpolation(const hardware_interface::JointTrajectory& trajectory)
{
  ...
  for (size_t i = 0; i < point_number; i++)
  {
    ...
    if(point.positions.size() == 6 + linker_hand_.joint_nums_) {
      for(int j = 0; i < linker_hand_.joint_nums_; j++) {
        linker_hand_.setangle_cmd_[j] = point.positions[6+j];
      }
    }
...
void URwLinkerHardwareInterface::startCartesianInterpolation(const hardware_interface::CartesianTrajectory& trajectory)
{
  size_t point_number = trajectory.trajectory.points.size();
...
void URwLinkerHardwareInterface::cancelInterpolation()
{
  ...
  for(size_t i = 0; i < linker_hand_.joint_nums_; i++) {
    linker_hand_.setangle_[i] = linker_hand_.curangle_[i];
  }
}

void URwLinkerHardwareInterface::passthroughTrajectoryDoneCb(urcl::control::TrajectoryResult result)
{
...
void URwLinkerHardwareInterface::handCommunicationThread(linker_hand::hand_comm& linker_hand, bool& in_freedrive, bool& power_open, bool& power_close) {
  while (ros::ok()) {
    if(power_open) {
      for(int i = 0; i < linker_hand.joint_nums_; i++) {
        linker_hand.setangle_[i] = linker_hand.angle_lower_limit[i];
      }
      if(in_freedrive) {
        power_open = false;
      }
      ...
      for(int i = 0; i < linker_hand.joint_nums_; i++) {
        linker_hand.setangle_[i] = linker_hand.angle_upper_limit[i];
      }
      if(in_freedrive) {
        power_close = false;
      }
    } else if(in_freedrive) {
      /* Force is not passed over so disable for now
      ...
      for(int i = 0; i < 6; i++) {
        if(fabs(linker_hand.setangle_[i] - linker_hand.curangle_[i]) < 0.05) {
          auto force_neg_threshold = force_neg_threshold_lookup[i];
          if(linker_hand.curangle_[i] > 0.5) {
            force_neg_threshold *= 2.0;
          }
          if(linker_hand.curforce_[i] > force_pos_threshold_lookup[i]) {
            linker_hand.setangle_[i] = std::max(linker_hand.setangle_[i] - force_ratio_lookup[i] * (linker_hand.curforce_[i] - force_pos_threshold_lookup[i]) / 2.0 , linker_hand::angle_lower_limit[i]);
          } else if(linker_hand.curforce_[i] < force_neg_threshold) {
            linker_hand.setangle_[i] = std::min(linker_hand.setangle_[i] - force_ratio_lookup[i] * (linker_hand.curforce_[i] - force_neg_threshold_lookup[i]), linker_hand::angle_upper_limit[i]);
          }
        }
      }
      */
    } else {
      for(int i = 0; i < linker_hand.curangle_.size(); i++) {
        linker_hand.setangle_[i] = linker_hand.setangle_cmd_[i];
      }
    }
    linker_hand.set_angle(linker_hand.setangle_);
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
  }
}
}  // namespace ur_driver

PLUGINLIB_EXPORT_CLASS(ur_driver::URwLinkerHardwareInterface, hardware_interface::RobotHW)
